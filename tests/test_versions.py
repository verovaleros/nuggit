...
from nuggit.api.routes.versions import get_repository_at_version, _compare_text as compare_text
...
class TestVersions(unittest.TestCase):
    ...
    def test_get_repository_at_version(self, mock_get_history, mock_get_repo):
        ...
        mock_get_repo.side_effect = asyncio.coroutine(lambda x: self.current_repo)

        # Mock the get_repository_history function
        mock_get_history.return_value = self.sample_history
        mock_get_history.side_effect = asyncio.coroutine(lambda x: self.sample_history)

        # Call the function
        repo_at_version = asyncio.run(get_repository_at_version(self.repo_id, self.version_timestamp))
        ...
        mock_get_repo.side_effect = asyncio.coroutine(lambda x: None)

        with self.assertRaises(Exception):
            asyncio.run(get_repository_at_version("nonexistent/repo", self.version_timestamp))

        # Test with a timestamp after all changes
        mock_get_repo.return_value = self.current_repo
        mock_get_repo.side_effect = asyncio.coroutine(lambda x: self.current_repo)

        repo_at_version = asyncio.run(get_repository_at_version(self.repo_id, "2023-01-03T00:00:00"))
